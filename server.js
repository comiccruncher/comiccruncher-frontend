...
const ACCESS_CONTROL = process.env.ACCESS_CONTROL ? process.env.ACCESS_CONTROL : 'https://comiccruncher.com';
...
app
  ...
  .then(() => {
    ...
    if (!IS_DEV) server.use(HTTPSRedirect);
    // Redirect www to non-www since we are using GAE standard.
    if (!IS_DEV) server.use(NonWWWRedirect);
